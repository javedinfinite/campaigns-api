...
const checkClosedCampaigns  = (endDate, procuredAmount, totalAmount) =>{
  ...
  const endingDate = new Date(endDate)
  const todayDate = new Date() 
  if( procuredAmount>=totalAmount || endingDate<todayDate )  
    return true
  else 
    return false
}
...
try{
  data_list.forEach((element) => {
    if(checkClosedCampaigns(element.endDate, element.procuredAmount, element.totalAmount))
    {
      let temp_object = {};
      temp_object.title = element.title,
      temp_object.totalAmount = element.totalAmount,
  ...
  if(closedCampaignList.length==0)
    res.status(200).json({data:[], message: `The number of closed campaigns are: NULL`});
  else
    res.status(200).json({data:closedCampaignList, message:'success'});
};